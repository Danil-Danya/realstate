                        ...
                        <div class="swiper-wrapper">
                            <div class="swiper-slide header__slide" v-for="(slide, index) in getAppartaments" :key="slide">
                                <div class="header__item">
                                    ...
                                    <h2 class="header__title" v-else>{{ slide.name.length > 15 ? slide.name.slice(0, 10) + '...' : slide.name }}</h2>
                                    <p class="header__info">{{ slide.beds }} bedrooms, {{ slide.baths }} bathsrooms, {{ slide.square }} square(ft) </p>
                                </div>
                                    ...
                                    <ul>
                                        <li v-if="slide.imgPaths.split(',').length > 1" class="header__link-bg">
                                            <router-link :to="`/apartament/${slide.name}`" class="header__link">
                                                <img :src="`http://localhost:5000/${slide.imgPaths.split(',')[1]}`" alt="Link" class="header__link-img">
