    ...
    methods: {
        ...
        async uploadImagesAndCreatePosts (isDraft) {
            ...
            const formDataFields = [
                'title', 'date', 'time', 'isActive', 'views', 'isDraft'
            ];
        ...
        addTags(event) {
            ...
            if (event.key === 'Enter') {
                const newTags = this.$refs.tags.value.split(',').map(tag => tag.trim());
