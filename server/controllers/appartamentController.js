...
class Appartament {
    async createAppartament (req, res) {
        try {
            ...
            const imagesData = appartamentUpload(
                imagesRequest, 
                appartamentRequest.unitReferense
            );

            appartamentRequest.imgPaths = `${ imagesData.imgPaths }`;
            appartamentRequest.imagesMemory = '2mb';

            console.log(appartamentRequest);
    ...
    async editAppartament (req, res) {
        try {
            ...
            if (deleted) {
                ...
                if (uploadPath.length > 0 && paths.length > 0) {
                    appartamentRequest.imgPaths = `${imagesData.imgPaths}, ${imgPaths}`;
                }
